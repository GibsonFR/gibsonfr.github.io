...
const replayInputElement = document.getElementById("rpl-input");
...
function init() {
    ...
    mapSelectElement.addEventListener("change", () => {
        const mapId = parseInt(mapSelectElement.value, 10);
        loadMap(mapId);
    });

    replayInputElement.addEventListener("change", onReplayFileSelected);
    ...
    const initialMapId = parseInt(mapSelectElement.value, 10);
...
function createStars() {
    ...
    for (let i = 0; i < starCount; i++) {
        // points aléatoires sur une grosse sphère
...
function onKeyDown(event) {
    ...
    if (k === "z" || k === "q" || k === "s" || k === "d"   // ZQSD (FR)
        || k === "w" || k === "a" || k === "s" || k === "d")  // WASD (EN)
    {
        exitOrbitToFree();
    }
...
function onReplayFileSelected(event) {
    ...
    reader.onload = fileEvent => {
        ...
        if (replay.mapId.toString() !== mapSelectElement.value) {
            mapSelectElement.value = replay.mapId.toString();
...
function updatePlayerFromSamples(mesh, samples, isP1, deltaTime) {
    ...
    const targetQuat = new THREE.Quaternion().setFromEuler(
        new THREE.Euler(0, yawRad, 0, "YXZ")
    );
    ...
    if (cameraMode === "p1" && player1) {
        ...
        if (lastCameraMode !== "orbitP1") {
            ...
            const desiredPos = new THREE.Vector3()
                .copy(orbitTarget)
                .addScaledVector(dir, ORBIT_DISTANCE);
