...
async function resolveSteamIdFromSearchQuery() {
    ...
    let match = players.find(p => String(p.steam_id) === searchQuery.trim());
    if (match) return match.steam_id;

    match = players.find(p => (p.username || "").toLowerCase() === q);
    if (match) return match.steam_id;

    match = players.find(p => (p.username || "").toLowerCase().includes(q));
    if (match) return match.steam_id;
...
async function loadPlayerPage() {
    ...
    if (!steamId) {
        headerEl.innerHTML = '<div class="text-red-400">Player not found.</div>';
        return;
    }

    const snapshotPlayers = await loadSnapshotPlayers();
    let playerRow = snapshotPlayers.find(p => String(p.steam_id) === String(steamId));
    ...
    let winPct = headerRow.win_rate;
    if (typeof winPct === "number" && winPct <= 1) {
        winPct = Number((winPct * 100).toFixed(1));
    }
    headerRow.win_rate = winPct ?? 0;

    const isPremiumPlayer = await isSteamIdPremium(steamId);
    headerEl.innerHTML = createHeaderCardHtml(headerRow, isPremiumPlayer);
    ...
    if (matchesTimeline && matchesTimeline.length) {
        ...
        const eloHintEl = document.getElementById("eloHint");
        if (!ratingsRows || !ratingsRows.length) {
            if (eloHintEl) {
                eloHintEl.textContent = "No data available.";
            }
        } else {
            ...
            if (eloHintEl) {
                eloHintEl.textContent = "No match IDs found for these points.";
            }
        }
    }
    ...
    for (const matchRow of recentMatches) {
        ...
        const winnerSide =
            matchRow.winner === 1 || matchRow.winner === "1"
                ? "p1"
