...
async function loadLinkedPlayer(user) {
  ...
  try {
    ...
    if (!steamId) {
      if (linkedElement) {
        linkedElement.textContent = "No linked Steam ID found for your Discord account.";
      }
    } else if (linkedElement) {
      linkedElement.innerHTML = `Linked to SteamID <b>${steamId}</b> · <a class="text-indigo-300 hover:underline" href="player.html?steam_id=${encodeURIComponent(steamId)}">Open your player page</a>`;
    }
    return { discordId, steamId };
  } catch (error) {
    console.error(error);
    if (linkedElement) {
      linkedElement.textContent = "Error while resolving linked profile.";
    }
    return { discordId: null, steamId: null };
  }
}
...
async function saveProfileIdentifiers(user, identifiers) {
  try {
    await supabaseClient
      .from("profiles")
      .upsert({ id: user.id, discord_id: identifiers.discordId || null, steam_id: identifiers.steamId || null }, { onConflict: "id" });
  } catch (error) {
...
async function loadProfile() {
  ...
  let { data, error } = await supabaseClient
    .from("profiles")
    .select("premium_until,is_premium,discord_id,steam_id")
    .eq("id", user.id)
    .maybeSingle();
  if (error) {
  ...
  if (!profileRow) {
    const { error: upsertError } = await supabaseClient.from("profiles").upsert({ id: user.id });
    if (upsertError) {
  ...
  const nameGuess = metadata.full_name || metadata.name || metadata.preferred_username || metadata.user_name || "—";
  const emailAddress = user.email || metadata.email || "—";
  const premiumUntilDate = profileRow && profileRow.premium_until ? new Date(profileRow.premium_until) : null;
  const hasActivePremium = premiumUntilDate && premiumUntilDate > new Date();
  const premiumText = hasActivePremium ? `Active (until ${premiumUntilDate.toISOString().slice(0, 10)})` : "Not active";
  const statusElement = document.getElementById("accStatus");
  if (statusElement) {
  ...
  if (nameElement) {
    nameElement.textContent = nameGuess;
  }
  if (emailElement) {
  ...
  if (premiumElement) {
    premiumElement.textContent = premiumText;
  }
  if (premStateElement) {
    premStateElement.textContent = premiumText;
  }
...
async function getPayPalPublicConfig() {
  ...
  const response = await fetch(`${SUPABASE_URL}/functions/v1/paypal-public-config`, {
    method: "GET",
...
function loadPayPalSdk(config, onReady) {
  ...
  const currency = config && config.currency ? config.currency : "EUR";
  if (!clientId) {
    showPayPalErrorNotes("PayPal is not configured (missing client id).");
    return;
  }
...
async function callCapture(orderId) {
  ...
  if (!response.ok) {
    throw new Error(jsonResponse.error || response.statusText || "Capture failed");
  }
  return jsonResponse;
}
  ...
  paypal.Buttons({
    ...
    onApprove: async (data, actions) => {
      ...
      try {
        ...
        if (messageElement) {
          messageElement.textContent = "30-day pass activated. Enjoy!";
        }
        setTimeout(() => {
          location.reload();
        }, 800);
  ...
  function renderButtons() {
    ...
    paypal.Buttons({
      ...
      createOrder: (data, actions) => {
        let amount = parseFloat(amountInput && amountInput.value ? amountInput.value : "3");
      ...
      onApprove: async (data, actions) => {
        ...
        const messageElement = document.getElementById("prem-msg");
        if (messageElement) {
          messageElement.textContent = "Thanks a lot for your support! (Donations do not grant extra features.)";
        }
      }
...
async function initializeAccountPage() {
  ...
  if (!config) {
    showPayPalErrorNotes("Unable to fetch PayPal public config.");
    return;
  }
