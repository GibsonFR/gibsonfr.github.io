...
function renderLeaderboard() {
    if (!sortedLeaderboardRows.length) return;
...
async function loadLeaderboardData(isReload) {
    ...
    try {
        ...
        if (!res.ok) {
            if (statusElement) statusElement.textContent = "Error loading snapshot";
            console.error("Failed to fetch leaderboard_snapshot.json", res.status);
            return;
        }
        ...
        const rows = players.map(p => {
            const steam_id = p.steam_id;
            const username = p.username || steam_id;
            const games = Number(p.games ?? 0) || 0;

            let winRatePercent = 0;
            if (typeof p.win_rate === "number") {
                winRatePercent = p.win_rate * 100;
            }

            return {
                steam_id,
                username,
                rank: p.rank_label || "Unranked",
                elo: typeof p.elo === "number" ? Math.round(p.elo) : p.elo,
                rp: p.rp ?? 0,
                games,
                win_rate: Number(winRatePercent.toFixed(1)),
            };
        });
        ...
        if (heroStats) {
            const totalLabel = totalPlayers.toLocaleString();
            heroStats.textContent = `Live snapshot · ${totalLabel} players · updated every few minutes.`;
        }
    } catch (error) {
        console.error("Error while loading leaderboard snapshot", error);
        if (statusElement) {
            statusElement.textContent = "Error loading snapshot";
        }
    }
}
...
async function loadRecentMatches() {
    ...
    try {
        ...
        <div class="text-slate-300 text-sm">${whenStr}</div>
        <div class="font-semibold">${escapeHtml(matchRow.p1_name || "P1")} vs ${escapeHtml(matchRow.p2_name || "P2")}</div>
        <div class="text-slate-400 text-sm">
          Winner: <span class="text-slate-100">${escapeHtml(winnerName)}</span> · Map ${matchRow.map_id ?? "-"}
        </div>
      </a>`;
